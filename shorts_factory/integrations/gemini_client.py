...
SCRIPT_SYSTEM_PROMPT = (
    ...
    "마세요 — 원본 영상의 출처국가는 언급하지 말고, 대신 '다이소 가면 무조건 사야 한다는데'처럼 어디서 "
    "발견됐는지가 아니라 지금 당장 왜 사야 하는지에 초점을 맞춘 후킹 문구로 표현하세요 (단, 실제로 다이소에 "
    "파는 제품이 아니라면 다이소를 문자 그대로 언급하지 말고 같은 느낌의 다른 표현을 쓰세요). "
