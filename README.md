...
- `source_strategy: viral_translate_dub` 채널의 상세보기에는 **레퍼런스 영상 더빙** 폼도
  나타납니다 — 해외 레퍼런스 영상 파일 하나를 올리면 자막/워터마크 위치 자동 크롭 →
  Gemini가 영상을 직접 보고 대본/상단카피 작성 → TTS 더빙 → 무음 구간 제거+배속 →
  참고영상 스타일 짧은 자막까지 자동으로 만들어서 클립 이력에 "업로드 대기(재고)"로
  올려줍니다. `GEMINI_API_KEY`(및 `TTS_PROVIDER=elevenlabs`를 쓸 경우
  `ELEVENLABS_API_KEY`)가 `.env`에 있어야 동작합니다.
